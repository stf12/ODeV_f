 ...
 *
 * \anchor fig20 \image html 20_embedded_app.png "Fig.20 - Anatomy of an embedded application" width=400px
 *
 ...
 *
 *
 * \subsection fw_architecture_sec Firmware architecture
 *
 ...
 * priority. It interacts with three other objects as displayed in \ref fig21 "Fig.21":
 *
 * \anchor fig21 \image html 21_fw_architecture_idea.png "Fig.21 - INIT task diagram"
 *
 ...
 *
 * While this is a very important part of the framework, \ref fig01 "Fig.1" displays the whole firmware architecture: a soft real-time multitasking system designed in multiple layers.
 *
 * \anchor fig01 \image html 1_fw_architecture.png "Fig.1 - Firmware architecture" width=900px
 *
 * \section sys_init System initialization
 *
 ...
 * - Initialize the Systick used by the STM32 HAL (by default set to 1ms)
 ...
 * task and the one with the highest priority. The INIT task is in charge of to complete the system initialization before entering the commands loop.
 *
 * \subsection applicatrion_context The Application Context
 ...
 *
 * \subsection task_hw_init Task's hardware initialization
 ...
 *
 * At this moment the hardware is initialized, all the peripherals are ready to be used, but usually they are not started yet. This will be done later
 * in the application task control loop, after the task creation.
 *
 * \subsection task_sw_init Task creation and software resources initialization.
 ...
 * in order to initialize the task specific software resources and to provide to the INIT task all the parameters needed to instantiate a FreeRTOS task.
 ...
 *
 * \subsection init_command_loop INIT task command loop
 * After the system initialization is done, the INIT task gives the control to the application task, and it enters a command loop. In this loop
 ...
 *
 * \anchor fig07 \image html 7_pwr_management_2.png "Fig.7"
 *
 ...
 * must contribute to the power efficiency of the system. The flow chart in \ref fig08 "Fig.8" display a generic implementation.
 *
 * \anchor fig08 \image html 8_pwr_management_3.png "Fig.8"
 *
 ...
 * reset its power mode delay switch bit (AManagedTask::m_xStatus `nDelayPowerModeSwitch`) and it suspends itself waiting for the INIT task to complete the power mode switch.
 * Otherwise (AManagedTask::m_xStatus `nPowerModeSwitchPending == 0`) the task set the power mode delay switch bit (AManagedTask::m_xStatus `nDelayPowerModeSwitch = 1`), then it execute a step
 ...
 * so the advanced error framework is disabled. The AED is initialized - IAEDInit() - before the application context with the application managed tasks are created.
 ...
 * objects activated as soon an error is detected. \ref fig11 "Fig.11" displays how this error framework is integrated in the INIT control loop.
 *
 * \anchor fig11 \image html 11_error_sequence.png "Fig.11"
 *
 ...
 * The highest priority level (4) is not used by the application task, because it is reserved to a special system task, the INIT task.
 * Each task is designed to respond to events generated by the MCU peripherals IRQ.
 *
 * | Index | IRQ line      | IRQ  priority  | Note                                                         |
 * | :---: | :------------ | :------------: | :-------------------------------------------------------     |
 * | 1     | TIM7_IRQn     | 15             | generates the HAL 1ms tick                                   |
 * | 7     | SysTick_IRQn  | 15             | generate the scheduler 1ms tick                              |
 * | 8     | WWDG_IRQn     | 5              | Software Watchdog                                            |
 * | 9     | TIM6_IRQn     | 14             | High frequency tick to generate the runtime stat in DEBUG    |
 * | 10    | PendSV_IRQn   | 15             | Used by FreRTOS                                              |
 *
 * Note that, despite the task priorities, the Cortex-M4 defines the IRQ priorities from the lowest 15 to the highest 0.
 *
 *
 * @todo TBD.
 *
