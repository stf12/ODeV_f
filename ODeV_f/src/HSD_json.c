...
static void (*HSD_free_function)(void *);

/* Private function prototypes -----------------------------------------------*/
static int32_t get_JSON_from_Device(COM_Device_t *device, char **serialized_string);
...
int32_t HSD_init_JSON(void * (*malloc_function)(size_t), void (*free_function)(void *))
{
  ...
  HSD_free_function = free_function;
  return 0;
}

int32_t HSD_free(void * mem)
{
  HSD_free_function(mem);
  return 0;
}
...
{
  return get_JSON_from_Device(Device, SerializedJSON);
}
