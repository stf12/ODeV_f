...
volatile COM_Device_t COM_device = {0};
...
{  
  ...
  COM_device.sensors[ii] = (COM_Sensor_t*)pvPortMalloc(sizeof(COM_Sensor_t));
  
  if(COM_device.sensors[ii] == NULL)
    return -1;
  
  memset( COM_device.sensors[ii], 0, sizeof(COM_Sensor_t));
