// Generated by imageconverter. Please, do not edit!

#ifndef BITMAPDATABASE_HPP
#define BITMAPDATABASE_HPP

#include <touchgfx/hal/Types.hpp>
#include <touchgfx/Bitmap.hpp>

const uint16_t BITMAP_DARK_BUTTONS_ROUND_EDGE_ICON_BUTTON_ID = 0; // Size: 60x60 pixels
const uint16_t BITMAP_DARK_BUTTONS_ROUND_EDGE_ICON_BUTTON_PRESSED_ID = 1; // Size: 60x60 pixels
const uint16_t BITMAP_DARK_BUTTONS_ROUND_EDGE_SMALL_ID = 2; // Size: 170x60 pixels
const uint16_t BITMAP_DARK_BUTTONS_ROUND_EDGE_SMALL_PRESSED_ID = 3; // Size: 170x60 pixels
const uint16_t BITMAP_DARK_ICONS_DOCUMENT_32_ID = 4; // Size: 29x32 pixels
const uint16_t BITMAP_DARK_ICONS_DONE_32_ID = 5; // Size: 32x32 pixels
const uint16_t BITMAP_DARK_ICONS_EXPAND_48_ID = 6; // Size: 46x46 pixels
const uint16_t BITMAP_DARK_ICONS_GO_BACK_32_ID = 7; // Size: 32x30 pixels
const uint16_t BITMAP_DARK_ICONS_GO_NEXT_32_ID = 8; // Size: 32x30 pixels
const uint16_t BITMAP_DARK_PAGEINDICATOR_DOT_INDICATOR_SMALL_HIGHLIGHT_ID = 9; // Size: 15x15 pixels
const uint16_t BITMAP_DARK_PAGEINDICATOR_DOT_INDICATOR_SMALL_NORMAL_ID = 10; // Size: 15x15 pixels
const uint16_t BITMAP_LEFT_ARROW_SMALL_ID = 11; // Size: 53x42 pixels
const uint16_t BITMAP_ODEV_F_LOGO_ID = 12; // Size: 266x147 pixels

namespace BitmapDatabase
{
#ifndef NO_USING_NAMESPACE_TOUCHGFX
using namespace touchgfx;
#endif

class BitmapData;
const touchgfx::Bitmap::BitmapData* getInstance();
uint16_t getInstanceSize();
}

#endif
